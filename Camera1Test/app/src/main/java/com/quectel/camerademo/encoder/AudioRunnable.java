...
public class AudioRunnable extends Thread {
    ...
    public static final String TAG = "CameraDemo.AR";
...
//            MediaRecorder.AudioSource.VOICE_RECOGNITION,
    ...
    private MediaMuxerRunnable mMediaMuxerRunnable;
    ...
    public AudioRunnable(MediaMuxerRunnable mediaMuxerRunnable) {
        mMediaMuxerRunnable = mediaMuxerRunnable;
        mCameraId = mediaMuxerRunnable.mCameraId;

        Log.v(TAG, "mMediaMuxerRunnable = " + mMediaMuxerRunnable + ", mCameraId = " + mCameraId);
...
//      audioFormat.setLong(MediaFormat.KEY_DURATION, (long)durationInMs );
    ...
    public void run() {
        ...
        int readBytes;
        Log.d(TAG, "enter audio thread");
        while (!isExit) {
            /*启动或者重启*/
            if (!isStart) {
                Log.d(TAG, "MediaCodec for audio is not start, start it ");
                stopMediaCodec();
                try {
                    //if (DEBUG)
                    Log.d(TAG, "audio -- startMediaCodec...");
                    startMediaCodec();
                } catch (IOException e) {
                    e.printStackTrace();
                    isStart = false;
                ...
                if (readBytes > 0) {
                   // if(DEBUG)
                        Log.d(TAG, "--- read audio data and encode ---mCameraId = " + mCameraId);
...
//                    if(DEBUG) Log.e("ang-->", "解码音频数据:" + readBytes);
                    try {
                       ...
                       // if (DEBUG)
                        Log.d(TAG, "encode audio data fail");
                        e.printStackTrace();
                    }
                } else {
                    //Log.d(TAG, "--- no audio data , tunel---mCameraId = " + mCameraId);
                }
            }
            /**/
        }
...
//                    mIsEOS = true;
        ...
        /*获取解码后的数据*/
//        final MediaMuxerRunnable muxer = mediaMuxerRunnable;
        if (mMediaMuxerRunnable == null) {
            //if (DEBUG)
                Log.e(TAG, "MediaMuxerRunnable is unexpectedly null, mCameraId = " + mCameraId);
            return;
        }
        ...
        do {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (mMediaMuxerRunnable != null) {
                    //if (DEBUG)
                        Log.d(TAG, "add audio track INFO_OUTPUT_FORMAT_CHANGED " + format.toString());
//                    mediaMuxerRunnable.addTrackIndex(MediaMuxerRunnable.TRACK_AUDIO, format);
                    mMediaMuxerRunnable.setMediaFormat(MediaMuxerRunnable.TRACK_AUDIO, format);
                }
                ...
                if (mBufferInfo.size != 0 && mMediaMuxerRunnable != null) {
                    mBufferInfo.presentationTimeUs = getPTSUs();
                    if(DEBUG)
                        Log.e(TAG, "add encoded audio data " + mBufferInfo.size);
                    mMediaMuxerRunnable.addMuxerData(MediaMuxerRunnable.TRACK_AUDIO, encodedData, mBufferInfo);
